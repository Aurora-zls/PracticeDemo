...
package com.example.zls.widget.custom_view.custom_text_view;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!isForbiddenActionMenu || mActionMenu.getChildCount() == 0) {
                    ...
                    if (isLongPress) {
                        ...
                        // 选择字符
                        Selection.setSelection(getEditableText(), Math
                                        .min(mStartTextOffset, mWordOffset_move),
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isLongPress) {
                    ...
                    Selection.setSelection(getEditableText(), Math
                                    .min(mStartTextOffset, mCurrentTextOffset),
...
//        canvas.restore();
            ...
            for (int i = 0; i < line_words.length; i++) {
                ...
                if (line_words.length == 1 || (isContentHanZi(word_i) && i < line_words.length - 1)) {
                    ...
                    for (int j = 0; j < word_i.length(); j++) {
                        ...
                        float word_i_char_j_width = StaticLayout
                                .getDesiredWidth(word_i_char_j, getPaint());
    ...
    private float calculatorCharPositionToLeft(int line, int charOffset) {
        ...
        float insert_blank = (mViewTextWidth - desiredWidth) / (line_str.length() - 1);
        // 计算当前字符左侧所有字符的宽度
        float allLeftCharWidth = StaticLayout
                .getDesiredWidth(text_str.substring(lineStart, charOffset), getPaint());
