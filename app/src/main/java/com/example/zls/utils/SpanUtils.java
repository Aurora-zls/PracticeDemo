...
public final class SpanUtils {
    ...
    public SpanUtils setBullet(@IntRange(from = 0) final int gapWidth) {
        return setBullet(0, 3, gapWidth,0);
    }
    ...
    public SpanUtils setBullet(@ColorInt final int color,
            ...
            @IntRange(from = 0) final int gapWidth, float lineSpacingExtra) {
    ...
    private void updateCharCharSequence() {
        ...
        if (bulletColor != COLOR_DEFAULT) {
            mBuilder.setSpan(
                    new CustomBulletSpan(bulletColor, bulletRadius, bulletGapWidth, lineSpacingExtra),
                    start,
                    end,
                    flag
            );
...
//            }
            ...
            canvas.drawText(text.toString(), x,
                    y - ((y + fm.descent + y + fm.ascent) / 2 - (bottom + top) / 2), paint);
        }
    }
    ...
    static class CustomLineHeightSpan implements LineHeightSpan {
        ...
        public void chooseHeight(final CharSequence text, final int start, final int end,
            ...
            Timber.e("start fm %s ----- sfm %s", fm, sfm);
            if (sfm == null) {
            ...
            Timber.e("end fm %s ----- sfm %s", fm, sfm);
        }
    }
        ...
        public void drawLeadingMargin(final Canvas c, final Paint p, final int x, final int dir,
            ...
            if (((Spanned) text).getSpanStart(this) == start) {
                ...
                if (c.isHardwareAccelerated()) {
                    ...
                    c.save();
                    c.translate(x + dir * radius, (top + bottom - lineSpacingExtra) / 2.0f);
                    c.drawPath(sBulletPath, p);
                    c.restore();
                } else {
                    c.drawCircle(x + dir * radius, (top + bottom - lineSpacingExtra) / 2.0f, radius,
                            p);
                }
                p.setColor(oldColor);
                p.setStyle(style);
